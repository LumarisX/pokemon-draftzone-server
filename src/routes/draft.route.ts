...
import { Matchup, Score, GameTime } from "../classes/matchup";
import {
  getScore,
  getStats,
...
export type DraftResponse = Response & {
...
const DraftRoutes: Routes = [
  ...
  {
    path: "/:team_id/:matchup_id/schedule",
    get: async (req: SubRequest, res: DraftResponse) => {
      if (!res.draft) {
        return;
      }
...
DraftRoutes.forEach((entry) => {
  const route = draftRouter.route(entry.path);
