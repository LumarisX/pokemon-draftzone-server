...
import { DraftPokemonMapper } from "./draft-pokemon.mapper";
...
describe("DraftPokemonMapper", () => {
  describe("fromDatabase", () => {
    ...
    it("builds a DraftPokemon with the entity's stored fields", () => {
      const result = DraftPokemonMapper.fromDatabase(buildEntity(), NAT_DEX);

      expect(result).toBeInstanceOf(DraftPokemon);
    ...
    it("leaves capt/modifiers undefined when the entity has none", () => {
      const result = DraftPokemonMapper.fromDatabase(
  ...
  describe("fromForm", () => {
    function buildDto(overrides: Partial<DraftPokemonDto> = {}): DraftPokemonDto {
      return {
        id: "charizard",
        name: "Charizard",
        shiny: false,
    ...
    it("builds a DraftPokemon with the form's submitted fields", () => {
      const result = DraftPokemonMapper.fromForm(buildDto(), NAT_DEX);
  ...
  describe("toDatabasePayload", () => {
    it("copies the domain object's fields into the persisted shape", () => {
      const pokemon = new DraftPokemon(
        {
          id: "pikachu",
          shiny: true,
          nickname: "Sparky",
      ...
      const result = DraftPokemonMapper.toDatabasePayload(pokemon);
  ...
  describe("toClientPayload", () => {
    it("includes the resolved name and passes through a partial Tera type list as-is", () => {
      const pikachu = new DraftPokemon(
        { id: "pikachu", capt: { tera: ["Water"] } },
        NAT_DEX,
      );

      const result = DraftPokemonMapper.toClientPayload(pikachu);
    ...
    it("leaves capt.tera undefined (not expanded) when no Tera type was set at all", () => {
      const pikachu = new DraftPokemon({ id: "pikachu" }, NAT_DEX);
    ...
    it("still returns a capt object when only dmax is set", () => {
      const pikachu = new DraftPokemon({ id: "pikachu", capt: { dmax: true } }, NAT_DEX);
