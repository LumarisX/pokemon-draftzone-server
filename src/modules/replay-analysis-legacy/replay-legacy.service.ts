...
import { Data, Generation, Generations, Move, Specie, toID } from "@pkmn/data";
...
@Injectable()
export class ReplayAnalyzerService {
  t0: number = 0;
  tf: number = 0;
  ...
  actionFns: { [key: string]: (line: ReplayLine) => void } = {
    ...
    "-burst": (line) => {
      const [pokemonStr, species, item] = line.args as [POKEMON, SPECIES, ITEM];
    },
    "-center": (line) => {
      const [] = line.args as [];
    },
    ...
    "-end": (line) => {
      ...
      if (effect.startsWith("move: ")) {
        ...
        this.setLastMove(
          this.isDelayedDamageMove(effect) && line.parent ? line.parent : line,
    ...
    "-fieldstart": (line) => {
      const [condition, majoraction] = line.args as [CONDITION, MARJORACTION?];
    ...
    drag: (line) => {
      ...
      const switchedMon = this.findTeamPokemonByDetails(switchPlayer, details);
      if (switchedMon) {
    ...
    replace: (line) => {
      ...
      let illusionMon = this.findTeamPokemonByDetails(illusionPlayer, details);
      ...
      illusionMon.damageDealt = {
        ...
        indirect:
          replaceMon.damageDealt.indirect - tempReplaceMon.damageDealt.indirect,
        teammate:
          replaceMon.damageDealt.teammate - tempReplaceMon.damageDealt.teammate,
      };
      ...
      illusionMon.damageTaken = {
        ...
        indirect:
          replaceMon.damageTaken.indirect - tempReplaceMon.damageTaken.indirect,
        teammate:
          replaceMon.damageTaken.teammate - tempReplaceMon.damageTaken.teammate,
      };
      // illusionMon.calcLog
  ...
  private getPokemonFromOfArg(ofArg: string | undefined): Pokemon | undefined {
  ...
  private buildStatusStart(line: ReplayLine, statusPosition: Pokemon): Status {
  ...
  private removeStatusByCondition(statuses: Status[], condition: string): void {
