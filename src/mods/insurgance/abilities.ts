...
export const Abilities: ModdedAbilityDataTable = {
  ...
  etherealshroud: {
    onTryHit(target, source, move) {
      if (
        move.category === "Status" ||
        source.hasAbility("scrappy") ||
        target === source
      )
        return;
      if (target.volatiles["miracleeye"] || target.volatiles["foresight"])
        return;
    ...
    onWeatherChange(pokemon) {
      ...
      switch (pokemon.effectiveWeather()) {
        ...
        case "snow":
          ...
          break;
        case "newmoon":
          if (pokemon.species.id !== "castformcloudy")
    ...
    onEnd(pokemon) {
      if (pokemon.illusion) {
        ...
        const details =
          ...
          (pokemon.level === 120 ? "" : ", L" + pokemon.level) +
  ...
  lernean: {
    ...
    onResidual(pokemon) {
      if (
        !pokemon.baseSpecies.id.includes("hydreigonmega") ||
        !pokemon.species.id.includes("hydreigonmega") ||
        !pokemon.hp
      ) {
        return;
      }
  ...
  omnitype: {
    ...
    onTryHit(target, source, move) {
      ...
      if (
        !source.hasAbility("irrelephant") &&
        !source.hasItem("ringtarget") &&
        immunities.includes(move.type)
      ) {
        this.add(
          "-immune",
          this.effectState.target,
          "[from] ability: Omnitype"
        );
        return null;
      }
      if (
  ...
  phototroph: {
    ...
    onResidual(pokemon) {
      if (
        ["raindance", "primordialsea", "newmoon"].includes(
  ...
  proteanmaxima: {
    onAfterMega(pokemon) {
      if (
        !pokemon.baseSpecies.id.includes("eeveemega") ||
        !pokemon.species.id.includes("eeveemega")
      ) {
        return;
      }
    ...
    onTryHit(target, source, move) {
      if (
        !target.baseSpecies.id.includes("eeveemega") ||
        !target.species.id.includes("eeveemega")
      ) {
        return;
      }
  ...
  regurgitation: {
    onAfterMove(pokemon, target, move) {
      ...
      if (
        pokemon.species.baseSpecies !== "Muk-Delta" ||
        pokemon.species.name === "Muk-Delta"
      )
        return;
      ...
      if (pokemon.species.name === "Muk-Delta-Normal")
        regurgMove.type = "Normal";
      if (pokemon.species.name === "Muk-Delta-Psychic")
        regurgMove.type = "Psychic";
      const regurgEffectiveness = this.dex.getEffectiveness(
        regurgMove.type,
        target
      );
      const regurgDamage = Math.floor(
        (2 ** regurgEffectiveness * target.baseMaxhp) / 6
      );
      regurgMove.damage = regurgDamage;
      if (
        move.name === "Regurgitation" ||
        !target.hp ||
  ...
  supercell: {
    ...
    onEnd(pokemon) {
      if (
        pokemon.species.name !== "Typhlosion-Delta-Mega-Active" ||
        !pokemon.hp
      )
        return;
      pokemon.formeChange("Typhlosion-Delta-Mega", this.effect, false);
