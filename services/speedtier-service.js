...
function speedTierChart(aTeam, bTeam) {
  let tiers = [];
  for (let m in aTeam) {
    tiers = tiers.concat(getSpeedTiers(aTeam[m], true));
  }
  for (let m in bTeam) {
    tiers = tiers.concat(getSpeedTiers(bTeam[m], false));
  }
...
function getSpeedTiers(pokemonId, isATeam) {
  let speedAbilities = []
  let tiers = [];
  let level = 50;
