...
function learns(pokemonId, moveId, gen) {
  if (LearnsetService.hasLearnset(pokemonId)) {
    ...
    if (
      "prevo" in Pokedex[pokemonId] &&
      learns(toKey(Pokedex[pokemonId].prevo), moveId, gen)
    ) {
      return true;
    }
    if (
      "changesFrom" in Pokedex[pokemonId] &&
      learns(toKey(Pokedex[pokemonId].changesFrom), moveId, gen)
    ) {
      return true;
    }
  } else if (
    "baseSpecies" in Pokedex[pokemonId] &&
    learns(toKey(Pokedex[pokemonId].baseSpecies), moveId, gen)
  ) {
    return true;
  }
  return false;
